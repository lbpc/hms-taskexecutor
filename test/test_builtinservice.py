...
class TestLinuxUserManager(TestCase):
    ...
    def test_default_shell(self):
        self.assertEqual(bs.LinuxUserManager().default_shell,
                         mock_config.CONFIG.builtinservice.linux_user_manager.default_shell)

    def test_disabled_shell(self):
        self.assertEqual(bs.LinuxUserManager().disabled_shell,
                         mock_config.CONFIG.builtinservice.linux_user_manager.disabled_shell)

    def test_get_user(self):
        etc_passwd_path = '{}/passwd'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_passwd_path, contents=dedent("""
        ...
        """).lstrip())
        etc_shadow_path = '{}/shadow'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_shadow_path, contents=dedent("""
    ...
    def test_create_group_existing(self):
        ...
        """).lstrip())
        bs.LinuxUserManager().create_group('u185131', gid=50700)
        self.assertEqual(self.fs.get_object(etc_group_path).contents, dedent("""
            u185128:x:34450:
            u185131:x:50700:
            u185130:x:29411:
        """).lstrip())
        self.assertEqual(self.fs.get_object(etc_gshadow_path).contents, dedent(""" 
            u185128:!::
            u185131:!::
            u185130:!::
        """).lstrip())
    ...
    def test_create_group_existing_with_members(self):
        ...
        """).lstrip())
        etc_gshadow_path = '{}/gshadow'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_gshadow_path, contents='group0:!::user0,user1,user2')
        etc_passwd_path = '{}/passwd'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_passwd_path, contents=dedent("""
                user0:x:1000:1000:Test User,,,:/home/user0:/bin/bash
                user1:x:1001:1001:Test User,,,:/home/user1:/bin/false
        ...
        self.assertEqual(self.fs.get_object(etc_group_path).contents, dedent("""
            ...
            group1:x:2000:user3
        """).lstrip())
        self.assertEqual(self.fs.get_object(etc_gshadow_path).contents, 'group0:!::user0,user1,user2')
    ...
    def test_create_group_no_gid(self):
        etc_group_path = '{}/group'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_group_path, contents=dedent(f"""
            group3:x:4:
            group0:x:1:
            group2:x:3:
            groupplusinf:x:{mock_config.CONFIG.builtinservice.linux_user_manager.min_uid}:
            group1:x:2:
        """).lstrip())
        bs.LinuxUserManager().create_group('group4')
        self.assertEqual(self.fs.get_object(etc_group_path).contents, dedent(f"""
            group3:x:4:
            group0:x:1:
            group2:x:3:
            groupplusinf:x:{mock_config.CONFIG.builtinservice.linux_user_manager.min_uid}:
    ...
    def test_create_group_existing_no_gid(self):
        etc_group_path = '{}/group'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_group_path, contents='testgroup:x:123123:')
        bs.LinuxUserManager().create_group('testgroup')
        self.assertEqual(self.fs.get_object(etc_group_path).contents, 'testgroup:x:123123:')
    ...
    def test_add_user_to_group(self):
        ...
        """).lstrip())
        etc_gshadow_path = '{}/gshadow'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        self.fs.create_file(etc_gshadow_path, contents=dedent(f"""
            group0:!::u223136
            group1:!::
        """).lstrip())
        mgr = bs.LinuxUserManager()
    ...
    def test_create_user(self, mock_time):
        ...
        self.assertEqual(self.fs.get_object(etc_passwd_path).contents, dedent("""
            u223135:x:80742:80742:account:/home/u223135:/bin/bash
            u223136:x:80743:80743:account:/home/u223136:/usr/sbin/nologin
            u223137:x:80744:80744:account:/home/u223137:/bin/bash
            u2000:x:2000:2000:account:/home/u2000:/bin/bash
        """).lstrip())
        self.assertEqual(self.fs.get_object(etc_shadow_path).contents, dedent("""
            u223135:$1$aRDLQJXb$TXKgBfCWPOKjFiMWfBXOW0:18224:0:99999:7:::
            u223136:$1$50i6mh7B$d7XLzlLdt0eAXaDPFtHwH/:18224:0:99999:7:::
            u223137:$1$5CHqbhOE$wEtod/g2KhiaZbbuPEWc4.:18224:0:99999:7:::
    ...
    def test_create_user_empty_files(self, mock_time):
        etc_passwd_path = '{}/passwd'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        etc_shadow_path = '{}/shadow'.format(mock_config.CONFIG.builtinservice.sysconf_dir)
        for each in (etc_passwd_path, etc_shadow_path): self.fs.create_file(each)
        mock_time.return_value = 1585905284.8418486
        mgr = bs.LinuxUserManager()
        mgr.create_user('u223135', 80742, '/home/u223135', '$1$aRDLQJXb$TXKgBfCWPOKjFiMWfBXOW0', '/bin/bash', 'account')
        self.assertEqual(self.fs.get_object(etc_passwd_path).contents, dedent("""
    ...
    def test_delete_user(self):
        ...
        self.assertEqual(self.fs.get_object(etc_passwd_path).contents, '')
        self.assertEqual(self.fs.get_object(etc_shadow_path).contents, '')
        self.assertEqual(self.fs.get_object(etc_group_path).contents, 'group03:x:9000:\n')
        self.assertEqual(self.fs.get_object(etc_gshadow_path).contents, 'group03:!::\n')
    ...
    def test_get_cpuacct(self):
        self.fs.create_file('{}/cpuacct/{}/u2000/cpuacct.usage'.format(
            mock_config.CONFIG.builtinservice.cgroupfs_mountpoint,
            mock_config.CONFIG.builtinservice.linux_user_manager.limitgroup
        ), contents='151714983162')
        mgr = bs.LinuxUserManager()
    ...
    def test_set_shell(self):
        ...
        self.fs.create_file(etc_passwd_path, contents=dedent("""
            u223135:x:80742:80742:account:/home/u223135:/bin/bash
        """).lstrip())
        self.fs.create_file('{}/shadow'.format(mock_config.CONFIG.builtinservice.sysconf_dir), contents=dedent("""
            u223135:$1$aRDLQJXb$TXKgBfCWPOKjFiMWfBXOW0:18224:0:99999:7:::
        """).lstrip())
