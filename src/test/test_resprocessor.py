...
class TestDatabaseProcessor(unittest.TestCase):
    @patch('taskexecutor.constructor.get_rescollector')
    @patch('taskexecutor.resprocessor.DatabaseProcessor.update')
    @patch('taskexecutor.resprocessor.ResProcessor._process_data')
    def test_create(self, mock_process_data, mock_update, mock_get_rescollector):
        db = Mock()
        db.name = 'b12345'
        mock_collector = Mock()
        mock_collector.get.return_value = db
        mock_get_rescollector.return_value = mock_collector
        mock_service = Mock(spec=DatabaseServer)
        processor = DatabaseProcessor(db, mock_service, {})
        processor.create()
        mock_get_rescollector.assert_called_once()
        mock_collector.get.assert_called_once()
        mock_service.create_database.assert_called_once_with('b12345')
        mock_process_data.assert_not_called()
        self.assertEqual(processor.op_resource, db)
